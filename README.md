...
There is a hosted service running in the background, which sets the active flag to false for bookings that have not been reserved within the 30 minutes. This service checks every minute. I wanted to create a controller to control the on/off state for the service but didn't get to it.
